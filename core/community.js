...
exports.labelPropagation = (numberOfNodes, _nodeSizeNeighborArray, iteration_limit = 100000000) => {
	...
	let counter=0; // variable for counting # of iteration
	...
	*/
	...
	while(counter<iteration_limit) { //break when can't get result in (iteration_limit) iterations 
		...
		// add counter for not found case
		counter++;
	}
...
exports.Louvain =(graph) =>{
	...
	*/
	...
	for(i=0; i<num_of_nodes; i++) //phase 1의 진행 순서를 randomize하기위한 randomized_node_list생성
	{
	...
	*/
	...
	{		
		node_list[randomized_node_list[i]]={
			/*
				NID : node ID(number)
				CID : community ID that this node place
			*/
	...
	{		
		edge_list[i]={
			/*
				edge : array of 2 elements like [NID,NID]
				weight : default is 1 because we only handle undirected graph
			*/
			edge : edges[i],
			weight : 1
		}
	...
	for(i=0; i<num_of_nodes; i++)//making community list
	{
		community_list[i]={
			/*
			nodes : array of NIDs which is in this community
			inner_weight : sum of weights which is absolutly in the community, default is 0 because there is just one nod in community at the first time
			total_weight : sum of (node size) in the community
			neighborCommunities : array of CIDs which is a neighbor of this array
			*/
	...
	{
		temp_community_edge=[node_list[edges[i][0]].CID,node_list[edges[i][1]].CID]; //tempoorary save edge
		is_in_array_result=this.isInEdgeArray(community_edge_list,temp_community_edge);
...
exports.isInEdgeArray=(community_edge_list,temp_community_edge)=>{
	...
	{
		if((community_edge_list[i].edge[0]==temp_community_edge[0])&&(community_edge_list[i].edge[1]==temp_community_edge[1]))
		{
			counter++;
			where=i;
		}		
